(function (window, document, $) {
	...
	App.prototype = {
		...
		repeatTarget: function () {

			jQuery.each(jQuery('[data-role="copies"]'), function () {
				var app = this;
				var $this = $(this);
				var qant = $this.data('qant');
				var maxqant = $this.data('max');
				var $hitter = $('[data-role="repeat-hitter"]');
				var $here = $('[data-here]');

				var hits = 1;

				$($('[data-target-name="referido"][data-id="1"]').find('[data-role="repeat-remover"]')).hide();

				if(!app.isLoading){
					app.isLoading = true;

					var $target = $('[data-target-name="'+ $hitter.data('target') +'"]');
					var $remover = $('[data-role="repeat-remover"]');

					$hitter.click(function (event) {
						event.preventDefault();

						var $html = $target.wrap('<div/>').parent().html();
						$target.unwrap();
						if(hits < maxqant){
							$here.before($html);
							hits = hits + 1;
							$this.attr('data-qant', hits);
						...
						var $last = $($('[data-target-name="referido"]').last());
						var $lastcounter = $last.find('[data-counter]');
						var $lastremover = $($last.find('[data-remove-id]'));
						$last.attr('data-id', hits);
						$lastremover.attr('data-remove-id', hits).show();
						$lastcounter.html(hits);


						$remover = $('[data-role="repeat-remover"]');

						// jQuery.each($remover, function () {
							$remover.click(function(innerevent){
								innerevent.preventDefault();
								var $este = $(innerevent.currentTarget);
								var $esteid = $este.data('remove-id');
								$este.parents('[data-id="'+ $esteid +'"]').remove();
								console.log('del: '+ hits);
								hits = hits - 1;
								$this.attr('data-qant', hits);
